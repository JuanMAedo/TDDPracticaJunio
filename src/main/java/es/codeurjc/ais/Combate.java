...
public class Combate {
    ...
    public static String combatir( List<Carta> atacantes, List <Carta> defensores){
        String resultado = "";
        if (defensores.isEmpty() || atacantes.get(0).getTablero() != defensores.get(0).getTablero()){
            return atacantes.get(0).toString() + " vs Nadie (Vacío) -> Daño directo de " +
                    atacantes.get(0).getAtaque()+ " punto(s).";
        }
        ...
        if(defensores.get(0).getDefensa() <= atacantes.get(0).getAtaque()){
            ...
            resultado += " (" + vidaRestante(defensores.get(0).getDefensa(),atacantes.get(0).getAtaque()) +
                    " punto(s) de Vida restante(s)).";
        }
        return resultado;
    }
